...
const ManageGallery = () => {
    ...
    const checkPermission = (id, action) => {
        ...
        const getMaingallery = gallery.slice(0, 6)
        const getService = getMaingallery.find(({_id}) => id === _id)
        ...
        swal({
          ...
          .then( wantDelete => {
            if (wantDelete) {
                ...
                axios.delete(`https://backend.ashinteriorbd.com/api/gallery/v1/gallery/list/${id}`)
                .then(res => {
    ...
    return (
        <>
        { edit ? 
            <AddGallery edit={edit} setEdit={setEdit} gallery={gallery}/> 
            : 
            gallery.length === 0 ?
            <TableLoader/>
            :
           <div className="orderList">
                <Table hover width="100%">
                    ...
                    <tbody>
                        {
                            gallery.map(({_id, title, subTitle, createdAt}) => {
