...
const AddTeam = ({edit, setEdit, teams}) => {
    
    const { register, handleSubmit, reset } = useForm();
    const [imgURL, setImgURL] = useState(null);
    const [header, setHeader] = useState({})
    const [isDisabled, setIsDisabled] = useState(false);
    const {name, designation, createdAt, photo} = header || {};

    useEffect(() => {
        const getService = teams?.find(({_id}) => _id === edit);
    ...
    const onSubmit = data => {
        ...
        if(edit){
            axios.put(`https://backend.ashinteriorbd.com/api/team/v1/team/update/${edit}`, headerInfo)
            .then(res =>{
            ...
            axios.post('https://backend.ashinteriorbd.com/api/team/v1/team/add', headerInfo)
            .then(res => {
                toast.dismiss(loading)
                if(res.data){
        ...
        <Form onSubmit={handleSubmit(onSubmit)} className="addServiceForm">
            <Row className="justify-content-center px-4">
                <Form.Group as={Col} md={7}>
                    <Form.Label style={{ fontWeight: "bold" }}>TeamMember Name</Form.Label>
                    <Form.Control
                        type="text"
                        defaultValue={name}
